defmodule Dotuh.VDF.Reader do
  alias Dotuh.VDF.ParseError

  import Dotuh.VDF.ReaderUtil
  ...
  defp parse_result(%{stack: [{nil, map}]}) do
    {:ok, map}
  end

  defp parse_result(%{stack: stack}) when length(stack) > 1 do
    ...
    case final_stack do
      [{nil, map}] -> {:ok, map}
      [{_key, map}] -> {:ok, map}
      _ -> {:error, %ParseError{message: "unexpected end of file while within nested block"}}
    end
  end
  ...
  defp action_line_type(:unknown, acc) do
    {:ok, acc}
  end

  defp action_line_type(:empty_value, acc) do
    # Handle standalone "" as an empty value for the most recent key
    case acc[:stack] do
